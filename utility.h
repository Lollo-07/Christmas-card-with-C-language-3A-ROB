...
void setConsoleDimensions(int cols, int lines) {
    ...
    itoa(cols,sCols,10);    //10    la base di conversione
...
void setConsoleDimensions11(int cols, int lines) {

    HANDLE console = GetStdHandle(STD_OUTPUT_HANDLE);

    // Imposta le dimensioni della console
    COORD size;
    size.X = cols; // Larghezza desiderata
    size.Y = lines;  // Altezza desiderata
